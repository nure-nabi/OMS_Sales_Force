...
class BranchState extends ChangeNotifier {
  ...
  clean() async {
    _isLoading = false;
    _branchList.clear();
    //await SetAllPref.branchDetails(value: BranchResponse.fromJson({}));
  ...
  late List<BranchModelAgent> _branchList = [];
  List<BranchModelAgent> get branchList => _branchList;
  set branchList(List<BranchModelAgent> value) {
    _branchList = value;
    notifyListeners();
  }
  ...
  getBranchListFrom() async {
    _companyDetail = await GetAllPref.companyDetail();
    try {
      BranchResponse model = await BranchAPI.branch(
        databaseName: _companyDetail.databaseName,
        agentDesc: _companyDetail.userName,
      );
      if (model.statusCode == 200) {
        branchList = model.data;
     //   navigator.pushReplacementNamed(branchPath);
      } else {
        branchList = [];
        navigator.pushReplacementNamed(homePagePath);
      }
  ...
  userSelectedBranch(BranchModelAgent branchInfo) async {
    final saveMovementState = context.read<SaveMovementState>();

   // await SetAllPref.branchDetails(value: branchInfo);
